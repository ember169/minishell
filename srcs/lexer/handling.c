...
{
	if (handle_double_op(input, ls, tok_ls))
		return ;
	if (input[ls->i] == '>')
		append_op(tok_ls, ls, TOK_REDIR_OUT, 1);
	else if (input[ls->i] == '<')
		append_op(tok_ls, ls, TOK_REDIR_IN, 1);
	else if (input[ls->i] == '|')
		append_op(tok_ls, ls, TOK_PIPE, 1);
	else if (input[ls->i] == '(')
		append_op(tok_ls, ls, TOK_PAREN_LEFT, 1);
	else if (input[ls->i] == ')')
		append_op(tok_ls, ls, TOK_PAREN_RIGHT, 1);
}

int	handle_double_op(char *input, t_lexer_state *ls, t_token **tok_ls)
{
	if (input[ls->i] == '>' && input[ls->i +1] == '>')
		return (append_op(tok_ls, ls, TOK_REDIR_APPEND, 2));
	if (input[ls->i] == '<' && input[ls->i +1] == '<')
		return (append_op(tok_ls, ls, TOK_HEREDOC, 2));
	if (input[ls->i] == '|' && input[ls->i +1] == '|')
		return (append_op(tok_ls, ls, TOK_OR, 2));
	if (input[ls->i] == '&' && input[ls->i +1] == '&')
		return (append_op(tok_ls, ls, TOK_AND, 2));
	return (0);
}

int	append_op(t_token **tok_ls, t_lexer_state *ls, t_token_type type, int size)
{
	t_token	*new;
